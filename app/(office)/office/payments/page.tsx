...
export default async function OfficePaymentsPage() {
  ...
  const canPostPayment = hasOfficePermission(actor.role, "payments.post");
  ...
  return (
    <>
      <OfficePageHeader
        ...
        action={canPostPayment && outstanding.length > 0 ? <a className="office-button office-button--accent" href="#record-payment"><Plus aria-hidden="true" /> Record payment</a> : undefined}
      />
      ...
      {canPostPayment ? <section className="office-panel office-panel--wide office-record-composer" id="record-payment"><div className="office-panel__head"><div><span className="office-eyebrow">New collection</span><h2>Allocate once. Audit always.</h2></div></div>{outstanding.length > 0 ? <OfficePaymentForm invoices={outstanding.map((invoice) => ({ id: invoice.id, label: `${invoice.number ?? invoice.id.slice(0, 8)} · ${invoice.contactName}`, meta: `${formatOfficeMoney(invoice.balanceMinor, invoice.currency)} due` }))} paidAt={getOfficeLocalDate()} /> : <div className="office-alert office-alert--warning"><CircleDollarSign aria-hidden="true" /><div><strong>No collectible invoice is available</strong><p>Create, review, and issue an invoice before recording a payment. Draft invoices cannot accept money.</p></div></div>}</section> : null}
    </>
  );
}
