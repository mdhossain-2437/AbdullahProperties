...
import { OfficeIssueInvoiceForm, PrintInvoiceButton } from "@/components/office/finance-forms";
...
import { getOfficeDatabaseHealth, getOfficeInvoice } from "@/features/office/repository";
  ...
  return (
    <>
      <OfficePageHeader
        ...
        action={<div className="office-page-actions"><Link className="office-button office-button--ghost" href="/office/invoices"><ArrowLeft aria-hidden="true" /> Invoices</Link><PrintInvoiceButton /></div>}
      />
      ...
      <section className="office-panel office-panel--wide office-record-composer office-print-hide">
        ...
        {invoice.payments.length > 0 ? <div className="office-table-wrap"><table className="office-table"><thead><tr><th>Receipt</th><th>Date</th><th>Method</th><th>Status</th><th>Allocated</th></tr></thead><tbody>{invoice.payments.map((payment) => <tr key={payment.allocationId}><td>{payment.receiptNumber ?? payment.paymentId.slice(0, 8)}</td><td>{formatOfficeDate(payment.paidAt)}</td><td>{payment.method.replaceAll("_", " ")}</td><td><OfficeStatusBadge status={payment.status} /></td><td>{formatOfficeMoney(payment.amountMinor, invoice.currency)}</td></tr>)}</tbody></table></div> : <div className="office-alert office-alert--warning"><CircleAlert aria-hidden="true" /><div><strong>No collection allocated</strong><p>Issued invoices become available in the payment register. Drafts cannot accept collections.</p></div></div>}
